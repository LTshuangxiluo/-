...
输入数据为 CSV 文件（训练集`train_data.csv`、测试集`test_data.csv`），需包含以下字段：
...
将训练集和测试集 CSV 文件放入代码同级目录，命名为`train_data.csv`和`test_data.csv`。
...
- 训练完成后，模型权重保存为`ss_gat_fraud_detection.pth`；
- 核心评估指标说明：
  - `Acc`（准确率）：整体识别正确率；
...
| `SS_GAT`                | 模型核心类，包含：1. 社会信号注意力融合层2. 多头 GAT 特征提取层3. 图级分类层 |
...
- 解决方案：手动下载对应 PyTorch/CUDA 版本的 whl 文件安装，参考[PyG 官方文档](https://pytorch-geometric.readthedocs.io/en/latest/notes/installation.html)。

### 2. 显存不足（GPU Out of Memory）

- 解决方案：
  1. 降低`hidden_channels`至 32；
  2. 减小`batch_size`（代码中默认 1，无需调整）；
  3. 使用 CPU 运行（速度较慢）。
...
1. 论文：《基于图神经网络与社会信号融合的金融诈骗检测与干预机制》
2. PyTorch Geometric 官方文档：https://pytorch-geometric.readthedocs.io/
3. Graph Attention Networks (GAT) 原论文：https://arxiv.org/abs/1710.10903
